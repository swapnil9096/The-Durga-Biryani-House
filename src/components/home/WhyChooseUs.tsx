      ...
      <div className="mt-12 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {features.map((f, i) => (
          <Reveal key={f.title} delay={i * 60}>
            <div className="flex h-full items-start gap-4 rounded-2xl border border-charcoal-100 bg-white p-6 transition-shadow hover:shadow-md">
              <span
                className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-maroon-50 text-2xl"
                aria-hidden="true"
              >
                {f.icon}
              </span>
              <div>
                <h3 className="text-lg font-semibold text-charcoal-900">{f.title}</h3>
